 ...
 *  vectors. This class uses the CSV described in the lecture notes
 *  (http://www.cs.tcd.ie/courses/baict/baim/ss/part2/ctinduction-4up.pdf,
 *  equation (5)) and (Sebastiani 2002).  CSV values are not
 *  probabilities but the function is monotonically increasing on the
 ...
 * @author  
 * @version <font size=-1>$Id: BVBayes.java,v 1.2 2004/03/29 14:29:33 luzs Exp $</font>
...
public class BVBayes
{
  ...
  private ProbabilityModel pm = null;
  /** 
   ...
   * CAVEAT: larger documents receive disproportionally large CSV's
   * simply by virtue of containing more terms. Document lenght
   * normalisation could be employed here to correct this
   * distortion. A simpler approach would be to base the probability
   * model on term events rather than document events, as described in
   * (Mitchell 97, ch 6).
   */
  public double computeCSV(String cat, ParsedNewsItem pni){
    /**  ********************  Lab 04: Exercise *********************
     * implement the CSV as specified above. 
     */ 
    return 1;
  }
  ...
  public static void main(String[] args) {
    try {
       ...
       * - obtain a classification status value (CSV) for each news item
       *   in the resulting ParsedText by calling computeCSV() [which you
       *   also must implement],
       * - store these CSVs along with each document's true
       *   classification in ( tc.evaluation.CSVTable), 
       * - perform hard classification by applying a threshold (or
       *   thresholding strategy, e.g. proportional thresholding) to the
       *   CSV results, and
       * - print a summary of evaluation results (see below) for the entire test 
       *   set at the end of processing.
       *
       * N.B.: this file won't compile unless CSVTable is implemented.
       */
      if (tstrategy.proportional)
        csvt.applyProportionalThreshold(f.pm.getCatGenerality(category));
      else // UCut strategy (RCut will not be implemented)
      ...
      System.err.println(" BVBayes corpus_list categ prob_model threshold\n");
      ...
      System.err.println(" threshold: a real number (for UCut thresholding) or the name of a"); 
      System.err.println("     thresholding strategy. Currently supported strategy:");
